...
__global__ void bika_conv2d_backward_wb_kernel(
    ...
    if (o >= O || c >= C || kidx >= K * K) return;

    int kh = kidx / K;
    int kw = kidx % K;
    int tid = threadIdx.x;

    const int param_idx = ((o * C + c) * K + kh) * K + kw;

    const float w  = weight[param_idx];
    const float bb = bias[param_idx];
    ...
    for (long long n = tid; n < N; n += blockDim.x) {
        ...
        const float go =
            grad_output[(((b * O + o) * Ho + h_out) * Wo) + w_out];
    ...
    for (int offset = blockDim.x / 2; offset > 0; offset >>= 1) {
        if (tid < offset) {
            red_w[tid] += red_w[tid + offset];
            red_b[tid] += red_b[tid + offset];
        }
        __syncthreads();
    }

    if (tid == 0) {
        grad_weight[param_idx] = red_w[0];
        grad_bias[param_idx] = red_b[0];
    }
}
...
std::vector<torch::Tensor> bika_conv2d_backward(torch::Tensor grad_output,
    ...
    dim3 gridWB(O, C, K * K);
    int threadsWB = 256;
    size_t sharedWB = 2 * threadsWB * sizeof(float);
