...
To resolve contextual ambiguities and capture multi-scale spatial features simultaneously, context aggregation architectures were developed. Chen et al.~\cite{chen2018deeplabv3plus} proposed DeepLabV3+, combining Atrous Spatial Pyramid Pooling (ASPP) with an encoder-decoder structure; by employing dilated convolutions at multiple sampling rates ($r = 6, 12, 18$) to expand the receptive field without increasing parameter counts, DeepLabV3+ achieved state-of-the-art performance with 89.0\% mIoU on PASCAL VOC 2012~\cite{everingham2012pascal} and 82.1\% mIoU on Cityscapes~\cite{cordts2016cityscapes}. Similarly, Zhao et al.~\cite{zhao2017pspnet} introduced the Pyramid Scene Parsing Network (PSPNet) to prevent global context misclassifications—such as confusing boats on water with land vehicles—by applying a 4-level Pyramid Pooling Module ($1\times1, 2\times2, 3\times3, 6\times6$) to aggregate sub-region features across multiple scales, obtaining 80.2\% mIoU on Cityscapes~\cite{cordts2016cityscapes} and 85.4\% mIoU on PASCAL VOC 2012~\cite{everingham2012pascal}.
...
Attention mechanisms have further advanced dense visual perception by capturing global, long-range dependencies across driving scenes. Xie et al.~\cite{xie2021segformer} introduced SegFormer, a lightweight, positional-encoding-free Transformer paired with an All-MLP decoder; by employing overlapping patch embeddings and spatial-reduction self-attention, SegFormer achieved 84.0\% mIoU with MiT-B5 on Cityscapes~\cite{cordts2016cityscapes} (and 76.2\% mIoU at 47.7 FPS with MiT-B0), as well as 51.8\% mIoU on ADE20K~\cite{zhou2017ade20k}. To eliminate the quadratic computational complexity $\mathcal{O}(H^2 W^2)$ of global self-attention while maintaining cross-window connectivity, Cao et al.~\cite{cao2022swinunet} proposed Swin-UNet, a pure U-shaped Transformer architecture replacing convolutional layers with Shifted Window (S-WMSA) self-attention blocks, achieving 79.13\% average Dice score on the Synapse Multi-Organ dataset~\cite{landman2015synapse} and 90.00\% Dice on ACDC. Addressing zero-shot class-agnostic segmentation, Kirillov et al.~\cite{kirillov2023segment} developed Segment Anything (SAM), training a heavy ViT image encoder paired with a prompt encoder and two-way transformer mask decoder on the 1.1-billion-mask SA-1B dataset to achieve 46.5 mIoU zero-shot on COCO~\cite{lin2014coco}.
