...
Liu et al.~\cite{liu2024kan} proposed Kolmogorov-Arnold Networks as a practical neural network architecture inspired by this theorem. The key different between KANs and traditional MLPs is illustrated in Figure~\ref{fig:kan_vs_mlp}.
...
For model optimization, we utilize variations of stochastic gradient descent. The Adam optimizer~\cite{kingma2015adam} computes adaptive learning rates for each parameter using estimates of first and second moments of the gradients, making it highly effective for complex architectures like KANs. To improve generalization, we also employ AdamW~\cite{LoshchilovH19}, which decouples weight decay from the gradient update, providing better regularization. During training, learning rates are dynamically adjusted using the OneCycleLR scheduler~\cite{smith2019superconvergence}, a super-convergence technique that anneals the learning rate from an initial value to a high maximum value and then down to a minimum, preventing the model from settling into sharp, suboptimal local minima.
