...
I, Tran Giang Son, certify that the internship report of Mr. Dao Xuan Quy is qualified to be presented in the Internship Jury 2025-2026.
...
Semantic segmentation is a critical component of autonomous driving perception, requiring both high accuracy and low latency for real-time deployment on both ARM and x86/CUDA hardwares. Traditional segmentation networks based on U-Net and Transformer architectures rely on Multi-Layer Perceptrons (MLPs) with fixed activation functions, limiting their representational flexibility. 
...
\begin{itemize}
    \item Optimizing and Evaluating U-KAN (UKAN), a U-Net style segmentation network that replaces standard MLP layers with KAN layers using learnable activation functions on edges, including multiple KAN variants: FasterKAN (RSWAf basis), ReLU-KAN, HardSwish-KAN, PWLO-KAN, and TeLU-KAN.
    \item Implementing and evaluating BiKA (Binarized KAN), a novel architecture that combines the KAN philosophy of per-connection learnable activations with Binary Neural Network (BNN) techniques, achieving extreme compression through 1-bit binarized weights and activations with custom CUDA kernels.
...
\label{sec:context}
\subsection{Context}
Semantic segmentation, the task of assigning a class label to every pixel in an image, is a foundational capability for autonomous driving perception systems. Accurate road segmentation enables autonomous vehicles to identify drivable surfaces, lane boundaries, sidewalks, and other critical scene elements in real time. The evolution of segmentation architectures has progressed from early fully convolutional networks (FCN)~\cite{long2015fcn} to the U-Net encoder-decoder paradigm~\cite{ronneberger2015unet}, and more recently to Transformer-based models such as SegFormer~\cite{xie2021segformer} and Swin-UNet~\cite{cao2022swinunet}.

However, all of these architectures share a fundamental design choice inherited from Multi-Layer Perceptrons (MLPs): they place fixed, predetermined activation functions (e.g., ReLU, GELU) at the \emph{nodes} of the network, while the \emph{edges} (connections) carry only learnable linear weights. The Kolmogorov-Arnold Representation Theorem~\cite{kolmogorov1957, arnold2009} suggests an alternative: placing learnable, potentially non-linear activation functions on the \emph{edges} themselves, allowing the network to learn both the weights and the optimal activation shape for each connection. This principle forms the foundation of Kolmogorov-Arnold Networks (KANs)~\cite{liu2024kan}.

Recent work has demonstrated the potential of KAN-based architectures in medical image segmentation~\cite{li2024ukan}, but their application to road scene segmentation and their compatibility with extreme model compression techniques such as Binary Neural Networks (BNNs)~\cite{qin2020bireal, liu2020reactnet} remain largely unexplored.

\subsection{Problem Statement}
Deploying segmentation models on autonomous vehicles poses two competing requirements. First, the model must achieve high segmentation accuracy to ensure safe navigation—missed road boundaries or misclassified obstacles can lead to catastrophic failures. Second, the model must operate within strict computational budgets imposed by edge hardware (e.g., embedded GPUs, FPGAs, or even microcontrollers), demanding low latency, small model size, and minimal memory bandwidth.

Standard full-precision segmentation networks (32-bit floating point) achieve high accuracy but require significant computational resources. Binary Neural Networks (BNNs) offer extreme compression by quantizing both weights and activations to 1-bit representations, but they suffer from severe accuracy degradation due to the limited expressiveness of the sign function used for binarization. The central challenge addressed in this thesis is: \emph{Can the KAN paradigm of learnable per-connection activations mitigate the accuracy loss inherent in binarized networks, while preserving their computational efficiency advantages?}

\subsection{Motivation}
This research is motivated by the need to bridge the gap between expressive, high-accuracy segmentation models and the extreme efficiency demands of real-time edge deployment. By investigating two complementary approaches—UKAN for full-precision KAN-augmented segmentation and BiKA for binarized KAN convolutions—we aim to establish a comprehensive understanding of how KAN integration affects the accuracy-efficiency Pareto frontier for road segmentation. Furthermore, by implementing knowledge distillation from UKAN to BiKA, we explore whether the rich representations learned by full-precision KAN layers can be effectively transferred to their binarized counterparts.

\section{Related Work}
\label{sec:related_work}
The development of semantic segmentation architectures has progressed through several paradigm shifts. Long et al.~\cite{long2015fcn} introduced Fully Convolutional Networks (FCN), demonstrating that classification CNNs could be adapted for dense prediction. Ronneberger et al.~\cite{ronneberger2015unet} proposed U-Net, whose symmetric encoder-decoder architecture with skip connections became the dominant paradigm for biomedical and subsequently autonomous driving segmentation.

Modern segmentation architectures have increasingly incorporated attention mechanisms. SegFormer~\cite{xie2021segformer} introduced a hierarchical Transformer encoder with efficient self-attention, while Swin-UNet~\cite{cao2022swinunet} adapted the shifted-window Transformer for U-shaped segmentation. DeepLabV3+~\cite{chen2018deeplabv3plus} combined atrous spatial pyramid pooling with encoder-decoder structures to capture multi-scale context.
...
On the model compression front, XNOR-Net~\cite{rastegari2016xnor} and Bi-Real Net~\cite{qin2020bireal} established foundations for BNNs by introducing real-valued skip connections to stabilize binary training. ReActNet~\cite{liu2020reactnet} further improved BNN accuracy through learnable activation shifts (RPReLU). IR-Net~\cite{qin2020irnet} proposed Libra-PB for balanced weight binarization, and AdaBin~\cite{tu2022adabin} introduced learnable per-channel output scaling.

Our work builds upon these foundations by: (1) adapting U-KAN for road segmentation with multiple efficient KAN variants, and (2) proposing BiKA, which fuses the KAN per-connection activation philosophy with state-of-the-art BNN techniques for extreme edge deployment.

\section{Thesis Objective} 
\label{sec: obj}
The primary objective of this thesis is to implement, evaluate, and compare two KAN-based segmentation architectures for road scene understanding. Specifically, we aim to: (1) adapt the UKAN architecture with multiple KAN linear variants for road segmentation on the BDD100K dataset, (2) design and implement BiKASegNet, a binarized segmentation network that realizes the KAN philosophy through per-connection binary thresholds, (3) establish a knowledge distillation pipeline from full-precision UKAN to binarized BiKA, and (4) benchmark both architectures against standard U-Net and Transformer baselines in terms of segmentation quality, model size, and inference speed.
...
\begin{itemize}
    ...
    \item \textbf{Chapter 2} discusses the technical background of Kolmogorov-Arnold Networks, the U-Net segmentation paradigm, Binary Neural Networks, and the KAN variant implementations.
    \item \textbf{Chapter 3} details the methodology, including the UKAN architecture, KAN layer integration, the BiKA binarized convolution design, and the knowledge distillation framework.
...
This chapter introduces the foundational concepts underlying KAN-based segmentation networks. We begin by providing a high-level overview of Kolmogorov-Arnold Networks (KANs) and Binary Neural Networks (BNNs), and then explain how Binarized Kolmogorov-Arnold Networks (BiKA) bridge these two domains. Following this overview, we detail the mathematical formulations of these architectures.

\section{Overview of Core Concepts}

Kolmogorov-Arnold Networks (KANs) have recently emerged as a compelling alternative to traditional Multi-Layer Perceptrons (MLPs), inspired by the Kolmogorov-Arnold representation theorem. In a standard MLP, the network learns fixed scalar weights on its connections (edges) and applies predetermined, fixed non-linear activation functions (such as ReLU or sigmoid) at its neurons (nodes). KANs invert this paradigm by placing learnable, non-linear activation functions directly on the network edges, while the nodes simply sum the incoming signals. In the original KAN implementation, these edge-centric activation functions are parameterized using flexible, continuous basis functions like B-splines. This architecture grants KANs exceptional representational flexibility and potential interpretability, as each connection can adapt to model complex univariate relationships. However, evaluating these continuous spline functions requires significant floating-point operations, introducing a computational overhead that heavily constrains the deployment of standard KANs on resource-limited hardware.

Binary Neural Networks (BNNs) address the hardware constraints of deep learning by pushing model quantization to its absolute theoretical limit. In a BNN, both the network weights and the intermediate feature activations are constrained to 1-bit representations, typically $\{-1, +1\}$, utilizing the sign function. This severe quantization completely eliminates the need for expensive floating-point multiply-accumulate (MAC) operations. Instead, convolutions and dense layers can be executed using highly efficient bitwise XNOR and popcount operations, drastically reducing memory bandwidth and latency on edge devices like FPGAs and embedded GPUs. Despite these immense computational advantages, the extreme information loss induced by binarizing continuous variables natively limits the expressiveness of BNNs. They often suffer from degraded predictive accuracy compared to full-precision networks and require specialized training methodologies, such as the Straight-Through Estimator (STE) and dense skip connections, to maintain stable gradient flow during backpropagation.
...
\subsection{KAN vs MLP Architecture}
Liu et al.~\cite{liu2024kan} proposed Kolmogorov-Arnold Networks as a practical neural network architecture inspired by this theorem. The key distinction between KANs and traditional MLPs is illustrated in Figure~\ref{fig:kan_vs_mlp}.
...
\end{tikzpicture}
\caption{Comparison of MLP (fixed activations on nodes, learnable weights on edges) vs KAN (learnable activation functions on edges, summation on nodes)~\cite{liu2024kan}.}
...
\section{Efficient KAN Variants}
...
\subsection{FasterKAN (RSWAf Basis)}
FasterKAN~\cite{fasterkan2024} replaces B-splines with the Reflectional Switch Activation Function (RSWAf). Given a set of grid points $\{g_0, g_1, \dots, g_{G-1}\}$ uniformly spanning $[g_{\min}, g_{\max}]$ and an inverse denominator parameter $\beta$, the basis function is:
\begin{equation}
...
\subsection{ReLU-KAN}
ReLU-KAN replaces the smooth basis with a piecewise-linear function using ReLU:
\begin{equation}
...
\subsection{HardSwish-KAN}
HardSwish-KAN uses the HardSwish activation as its basis function:
\begin{equation}
    \psi_k(x) = \text{HardSwish}(x - g_k) = (x - g_k) \cdot \frac{\text{ReLU6}(x - g_k + 3)}{6}
\end{equation}
...
\subsection{TeLU-KAN}
TeLU-KAN employs the Hyperbolic Tangent Exponential Linear Unit:
\begin{equation}
    \psi_k(x) = (x - g_k) \cdot \tanh(\exp(x - g_k))
\end{equation}
...
\subsection{PWLO-KAN (Piecewise Linear Optimisation)}
PWLO-KAN implements a lookup-table-based approach where each segment of the input domain is associated with a learned affine transformation:
\begin{equation}
    \phi_k(x) = a_k \cdot x + b_k, \quad x \in [g_k, g_{k+1})
\end{equation}
where $a_k, b_k$ are learnable parameters stored as embedding tables. The segment index is computed as:
\begin{equation}
    k = \text{clamp}\left(\left\lfloor \frac{x - g_{\min}}{g_{\max} - g_{\min}} \cdot G \right\rfloor, 0, G-1\right)
\end{equation}
...
\section{Knowledge Distillation: UKAN to BiKA}
\label{sec:distillation}
To recover accuracy lost through binarization, we employ a knowledge distillation (KD) framework where a pre-trained full-precision UKAN model serves as the teacher and BiKASegNet serves as the student.

\subsection{Distillation Loss}
The total training loss for the student combines the standard task loss with a distillation loss:
\begin{equation}
    \mathcal{L}_{\text{total}} = \mathcal{L}_{\text{task}} + \lambda_{\text{KD}} \cdot \mathcal{L}_{\text{KD}}
\end{equation}

The task loss is the CrossEntropyDice loss applied to the student's predictions. The distillation loss computes the KL-divergence between softened probability distributions of the teacher and student:
\begin{equation}
    \mathcal{L}_{\text{KD}} = \frac{\tau^2}{B \cdot H \cdot W} \sum_{i \in \Omega} \text{KL}\left(\sigma\left(\frac{\mathbf{z}_t^{(i)}}{\tau}\right) \,\middle\|\, \sigma\left(\frac{\mathbf{z}_s^{(i)}}{\tau}\right)\right)
\end{equation}
where $\mathbf{z}_t^{(i)}$ and $\mathbf{z}_s^{(i)}$ are the teacher and student logits at pixel $i$, $\tau$ is the temperature parameter, and $\sigma(\cdot)$ denotes softmax. The normalization by $B \cdot H \cdot W$ ensures that the KD gradient magnitude is comparable to the per-pixel CE loss, preventing gradient scale mismatch.

\subsection{Auxiliary Boundary Loss}
An optional boundary loss is applied to improve the model's sensitivity to thin road boundaries and edges:
\begin{equation}
    \mathcal{L}_{\text{boundary}} = \text{BCE}\left(\Delta_{\text{logit}} \cdot M_{\text{boundary}}\right)
\end{equation}
where $\Delta_{\text{logit}}$ is the difference between the maximum and second-maximum class logits, $M_{\text{boundary}}$ is a binary boundary mask obtained by morphological erosion and dilation of the ground-truth segmentation, and BCE is the binary cross-entropy loss. This focuses learning on the topologically critical boundary pixels.

\subsection{Bipolar Regularisation}
To tighten the binarization gap between the latent FP32 weights and their binary projections, an optional bipolar regularisation term is added:
\begin{equation}
    \mathcal{L}_{\text{bipolar}} = \lambda_{\text{bp}} \sum_{\ell} \frac{1}{|\mathbf{w}_\ell|} \sum_{i} (1 - w_{\ell,i}^2)
\end{equation}
This penalty pushes the continuous latent weights toward $\pm 1$, reducing the quantization error of the sign function.
