...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Input Resolutions}: Default low-resolution grid $192 \times 256 \times 3$, medium resolution $360 \times 640 \times 3$, KAN grid resolution $384 \times 640 \times 3$, and full-resolution baseline grid $512 \times 1024 \times 3$.
...
The primary evaluation metrics are mean Intersection over Union (mIoU), per-class Road IoU ($\text{IoU}_0$), and mean Dice coefficient, computed via a confusion-matrix-based epoch-level accumulator over the full 1,000 validation images of BDD100K to avoid batch-averaging inaccuracies.
