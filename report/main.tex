...
\begin{table}[htbp]
\centering
\caption{Side-by-side structural and computational comparison between 16-channel and 32-channel BiKASegNet configurations.}
\label{tab:bika_channel_comparison}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lcc}
\hline
\textbf{Architectural / Hardware Parameter} & \textbf{16-Channel Configuration (\texttt{base\_channels = 16})} & \textbf{32-Channel Configuration (\texttt{base\_channels = 32})} \\ \hline
Encoder Stage Channels & $C \rightarrow 2C \rightarrow 4C \rightarrow 8C \quad (16 \rightarrow 32 \rightarrow 64 \rightarrow 128)$ & $C \rightarrow 2C \rightarrow 4C \rightarrow 8C \quad (32 \rightarrow 64 \rightarrow 128 \rightarrow 256)$ \\
Decoder Stage Channels & $12C \rightarrow 4C \rightarrow 6C \rightarrow 2C \rightarrow 3C \rightarrow C \quad (192 \rightarrow 64 \dots)$ & $12C \rightarrow 4C \rightarrow 6C \rightarrow 2C \rightarrow 3C \rightarrow C \quad (384 \rightarrow 128 \dots)$ \\
Total Model Parameters & $\sim 0.35\text{M}$ parameters & $\sim 4.1\text{M}$ parameters (matches UKAN capacity) \\
Binary Layer Memory Footprint & $\sim 1.4\text{ MB}$ (ultra-lightweight) & $\sim 4.8\text{ MB}$ ($\sim 32\times$ smaller than FP32 equivalent) \\
32-Bit Word Alignment & Partial ($16 \times 3 \times 3 = 144$ elements = 4 words $+$ 16-bit remainder) & Exact ($32 \times 3 \times 3 = 288$ elements = 9 full 32-bit words) \\
CUDA XNOR/Popcount Execution & Requires bit-masking \texttt{(\textasciitilde(x \^{} w)) \& mask} for tail remainder & Pure 32-bit full-word \texttt{\_\_popc()} execution across all layers \\
Representational Expressiveness & Ultra-compact for basic road masks & High capacity for fine lane boundaries and distant obstacles \\
Target Deployment Platform & Sub-watt microcontrollers \& ultra-low VRAM FPGAs & Edge GPUs (Jetson Nano/Orin) \& embedded accelerators \\ \hline
\end{tabular}
}
\end{table}
