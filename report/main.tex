...
\subsection{FastKAN (Gaussian RBF Basis)}
FastKAN~\cite{li2024fastkan} reframes Kolmogorov-Arnold Networks through the lens of Radial Basis Function (RBF) networks. It replaces the computationally intensive 3rd-order B-splines with Gaussian RBF basis functions defined over a uniform grid of center points $\{g_0, g_1, \dots, g_{G-1}\}$:
\begin{equation}
...
\subsection{ReLU-KAN}
ReLU-KAN~\cite{qiu2024relukan} replaces the smooth basis with a piecewise-linear function using ReLU:
\begin{equation}
...
HardSwish-KAN adapts the HardSwish activation~\cite{howard2019searching} as its basis function:
\begin{equation}
...
\subsection{TeLU-KAN}
TeLU-KAN employs the Hyperbolic Tangent Exponential Linear Unit (TeLU) activation~\cite{fernandez2024telu}:
\begin{equation}
...
PWLO-KAN draws upon piecewise linear network representations~\cite{schoots2025relating} and implements a lookup-table-based approach where each segment of the input domain is associated with a learned affine transformation:
\begin{equation}
...
\section{Optimization and Activation Functions}
\label{sec:optim_activations}
The successful training and efficient deployment of KAN-based architectures rely on specialized optimization techniques and functional approximations designed to mitigate the computational overhead of edge-centric activations.

\subsection{KAN Optimization and Acceleration}
While original KAN implementations suffer from slow training speeds due to iterative B-spline evaluations, several structural optimization methods have been proposed to accelerate computation. FastKAN~\cite{li2024fastkan} demonstrates that 3rd-order B-splines can be effectively approximated using Gaussian Radial Basis Functions (RBFs), reformulating KANs into RBF networks to eliminate grid iteration overhead and drastically accelerate forward and backward passes. In parallel, PowerMLP~\cite{qiu2024powermlp} addresses the training latency bottleneck by introducing non-iterative power-basis expansions, offering MLP-like execution speeds (achieving up to $40\times$ faster training than standard KANs) while preserving higher representational expressiveness. In our training pipelines, parameter optimization is further supported by adaptive gradient methods including Adam~\cite{kingma2015adam} and AdamW~\cite{LoshchilovH19}, alongside the OneCycleLR scheduler~\cite{smith2019superconvergence} for super-convergence.

\subsection{Activation Functions}
Activation functions inject the necessary non-linearity into the network. Beyond the standard Rectified Linear Unit (ReLU), our KAN variants utilize more advanced smooth activations. The Sigmoid Linear Unit (SiLU or Swish)~\cite{ramachandran2018searching}, defined as $x \cdot \sigma(x)$, is used extensively as a base activation function across the KAN linear layers due to its smooth, non-monotonic properties that improve gradient flow. Additionally, Hardswish~\cite{howard2019searching} is employed in specific KAN variants (HardSwish-KAN) to approximate the smooth SiLU activation with piecewise linear functions, significantly reducing computational overhead on edge devices while maintaining accuracy.
