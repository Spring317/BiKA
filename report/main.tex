...
\begin{itemize}
    ...
    \begin{itemize}
      \item \textbf{Input shape}: 192x265x3, 360x460x3
...
Table~\ref{tab:segmentation_results} summarizes the segmentation accuracy of full-precision UKAN, binarized BiKASegNet, and baseline architectures evaluated on BDD100K road segmentation.
...
BiKASegNet + KD & 1-bit & Binary Step & \textbf{84.15} & \textbf{91.39} \\
\bottomrule
\end{tabular}
\vspace{1ex}

{\small \textit{*Note: KD = Knowledge Distillation ($\tau=4$) trained with UKAN-FasterKAN as teacher on BDD100K.}}
...
Table~\ref{tab:efficiency_results} compares the model size, storage footprint, inference throughput (FPS), and overall compression ratios across all evaluated architectures.
...
\midrule
BiKASegNet (1-bit, Ours V8) & \textbf{0.97} & \textbf{0.88} & \textbf{659.37} & \textbf{3.63} & \textbf{141.05$\times$} \\
\bottomrule
\end{tabular}
\vspace{1ex}

{\small \textit{*Note: Storage size is measured after 1-bit weight packing. Compression ratio is computed relative to full-precision U-Net (31.03M FP32). FPS measured on NVIDIA RTX 2080 Max-Q GPU and Intel Core i7 CPU (12 threads).}}
...
The binary convolution operations in \texttt{BiKA\_Conv2d} leverage XNOR and popcount operations through the custom CUDA kernel. Table~\ref{tab:latency_profile} profiles the per-layer latency contributions.
...
{\small \textit{*Note: Latency measured as mean over 1000 forward passes at $360 \times 640$ input resolution on NVIDIA Turing GPU.}}
\caption{Per-component inference latency comparison between UKAN (FP32) and BiKASegNet (1-bit).}
\label{tab:latency_profile}
\end{table}

\subsection{GPU and CPU Optimization Journeys}
To analyze the progression of throughput improvements, Table~\ref{tab:gpu_optimization_journey} details the GPU optimization stages for BiKASegNet CUDA kernel development, while Table~\ref{tab:cpu_optimization_journey} illustrates the x86\_64 AVX2 SIMD CPU optimization journey.
...
{\small \textit{*Note: FPS measured at $360\times 640$ resolution on NVIDIA Turing GPU.}}
...
{\small \textit{*Note: Evaluated on Intel Core i7-10850H CPU (12 threads) at $360\times 640$ resolution.}}
...
{\small \textit{*Note: $\Delta$ mIoU is computed relative to the un-distilled 1-bit baseline.}}
...
{\small \textit{*Note: Cumulative ablation showing accuracy gains from BNN optimization techniques.}}
