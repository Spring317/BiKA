...
To overcome global memory bandwidth limitations during weight transfers, 32 continuous binary weights along kernel and input channel dimensions are bit-packed into single 32-bit unsigned integers (\texttt{unsigned int packed\_weight}). Bit-packing compresses the weight memory footprint by $32\times$, allowing 32 binary synapses to be fetched from GPU VRAM in a single 32-bit memory transaction.

\subsubsection{32-Bit Word Alignment and Channel-32 Hardware Optimisation}
Because CUDA warp execution processes 32 threads concurrently and hardware popcount (\texttt{\_\_popc}) operates on 32-bit unsigned integer words, filter weight dimensions that are exact multiples of 32 eliminate tail remainder masking (\texttt{bit < 32}). In \texttt{bika\_conv2d.cu}, weights are packed into 32-bit words along the $C \cdot K_H \cdot K_W$ dimension. When \texttt{base\_channels = 32}, each $3\times3$ binary convolution kernel processes $32 \times 3 \times 3 = 288$ elements, which packs into exactly $9$ full 32-bit words ($288 / 32 = 9$). This eliminates partial bit-masking in thread registers, executing pure full-word bitwise XNOR and \texttt{\_\_popc} across every single convolutional stage in the network.
...
During convolution execution, thread registers compare packed 32-bit input activation words (\texttt{x\_bits}) against packed binary weight words (\texttt{w\_bits}) using bitwise XNOR, followed by the hardware-accelerated population count intrinsic (\texttt{\_\_popc}):
\begin{equation}
