...
The field of semantic segmentation—and specifically road scene perception for autonomous driving—has evolved through several architectural paradigms over the past decade. This section reviews prior work across six key domains: foundational convolutional encoder-decoder networks, context aggregation architectures, real-time/lightweight road segmentation models, Vision Transformers, Kolmogorov-Arnold Networks (KANs), and Binary Neural Networks (BNNs).

The evolution of semantic segmentation began with the adaptation of classification Convolutional Neural Networks (CNNs) for dense pixel-level prediction. Long et al.~\cite{long2015fcn} introduced Fully Convolutional Networks (FCN), replacing fully connected layers with $1\times1$ convolutions to preserve spatial coordinates and enable variable-resolution image input; by fusing deep, coarse semantic features with shallow, fine spatial details via transposed convolutions (FCN-8s), FCN achieved 62.2\% mean Intersection over Union (mIoU) on PASCAL VOC 2012 and 34.0\% mIoU on NYUDv2. To address the loss of fine-grained spatial localization caused by repetitive pooling operations, Ronneberger et al.~\cite{ronneberger2015unet} proposed U-Net, a symmetric encoder-decoder architecture that concatenates high-resolution encoder feature maps directly to decoder stages across horizontal skip connections, establishing a dominant paradigm in biomedical and road segmentation while achieving 92.0\% IoU on the ISBI Cell Tracking Challenge. Seeking to minimize memory consumption during upsampling, Badrinarayanan et al.~\cite{badrinarayanan2017segnet} developed SegNet, which records max-pooling indices during encoding and reuses them in the decoder to perform sparse parameter-free unpooling, reaching 60.1\% mIoU on CamVid and 57.0\% mIoU on Cityscapes.

To resolve contextual ambiguities and capture multi-scale spatial features simultaneously, context aggregation architectures were developed. Chen et al.~\cite{chen2018deeplabv3plus} proposed DeepLabV3+, combining Atrous Spatial Pyramid Pooling (ASPP) with an encoder-decoder structure; by employing dilated convolutions at multiple sampling rates ($r = 6, 12, 18$) to expand the receptive field without increasing parameter counts, DeepLabV3+ achieved state-of-the-art performance with 89.0\% mIoU on PASCAL VOC 2012 and 82.1\% mIoU on Cityscapes. Similarly, Zhao et al.~\cite{zhao2017pspnet} introduced the Pyramid Scene Parsing Network (PSPNet) to prevent global context misclassifications—such as confusing boats on water with land vehicles—by applying a 4-level Pyramid Pooling Module ($1\times1, 2\times2, 3\times3, 6\times6$) to aggregate sub-region features across multiple scales, obtaining 80.2\% mIoU on Cityscapes and 85.4\% mIoU on PASCAL VOC 2012.

Deploying semantic segmentation models on autonomous vehicles requires high processing throughput under strict computational and thermal constraints. Paszke et al.~\cite{paszke2016enet} designed ENet, an ultra-compact real-time architecture utilizing bottleneck blocks with early spatial downsampling, factorized asymmetric $5\times1$ and $1\times5$ convolutions, and dilated kernels to maintain receptive fields with minimal parameters, reaching 58.3\% mIoU at 76.8 frames per second (FPS) on an NVIDIA Titan X for $1024\times512$ Cityscapes images and 51.3\% mIoU on CamVid. To overcome the speed-accuracy trade-off inherent in aggressive downsampling, Yu et al.~\cite{yu2018bisenet} proposed BiSeNet (Bilateral Segmentation Network), which decouples spatial detail preservation from context extraction into a shallow Spatial Path and a deep Context Path with Attention Refinement Modules; BiSeNet achieved an impressive 105.8 FPS with 74.7\% mIoU on Cityscapes ($768\times1536$ resolution) and 68.7\% mIoU on CamVid at 146.7 FPS.

Attention mechanisms have further advanced dense visual perception by capturing global, long-range dependencies across driving scenes. Xie et al.~\cite{xie2021segformer} introduced SegFormer, a lightweight, positional-encoding-free Transformer paired with an All-MLP decoder; by employing overlapping patch embeddings and spatial-reduction self-attention, SegFormer achieved 84.0\% mIoU with MiT-B5 on Cityscapes (and 76.2\% mIoU at 47.7 FPS with MiT-B0), as well as 51.8\% mIoU on ADE20K. To eliminate the quadratic computational complexity $\mathcal{O}(H^2 W^2)$ of global self-attention while maintaining cross-window connectivity, Cao et al.~\cite{cao2022swinunet} proposed Swin-UNet, a pure U-shaped Transformer architecture replacing convolutional layers with Shifted Window (S-WMSA) self-attention blocks, achieving 79.13\% average Dice score on the Synapse Multi-Organ dataset and 90.00\% Dice on ACDC. Addressing zero-shot class-agnostic segmentation, Kirillov et al.~\cite{kirillov2023segment} developed Segment Anything (SAM), training a heavy ViT image encoder paired with a prompt encoder and two-way transformer mask decoder on the 1.1-billion-mask SA-1B dataset to achieve 46.5 mIoU zero-shot on COCO.

Recently, Kolmogorov-Arnold Networks (KANs) were introduced by Liu et al.~\cite{liu2024kan} as a fundamental alternative to traditional Multi-Layer Perceptrons (MLPs). Grounded in the Kolmogorov-Arnold Representation Theorem~\cite{kolmogorov1957}, KANs replace fixed node activation functions (e.g., ReLU, GELU) with learnable 1D B-spline activation functions on network edges ($\phi(x) = w_b b(x) + w_s \text{spline}(x)$), enabling the network to dynamically learn activation function shapes while outperforming MLPs with up to $10\times$ fewer parameters on PDE solving and function approximation. Extending KANs to dense vision tasks, Li et al.~\cite{li2024ukan} introduced U-KAN (UKAN), reformulating 1D spline KAN operations into vectorized 2D matrix multiplications to replace standard MLP/Transformer blocks within a U-Net backbone, attaining an 89.84\% average Dice score on the Synapse Multi-Organ dataset and 90.12\% IoU on ISIC 2018 Skin Lesion Segmentation. To resolve the slow GPU training latency caused by non-uniform B-spline grid allocations in KANs, Tsiamis~\cite{fasterkan2024} developed FasterKAN, replacing grid-allocated B-splines with Reflectional Switch Activation Functions (RSWAf) based on multiscale Radial Basis Functions (RBF) to achieve a $3\times$ to $5\times$ speedup on CUDA platforms.

To achieve extreme model compression and speed for edge hardware deployment, Binary Neural Networks (BNNs) constrain both weights and activations to 1-bit binary values ($\{-1, +1\}$). Hubara et al.~\cite{hubara2016binarized} formalized 1-bit neural networks using the sign binarization function $x_b = \text{sign}(x)$ in the forward pass and the Straight-Through Estimator (STE) during backpropagation, replacing expensive 32-bit floating-point multiply-accumulate operations with bitwise XNOR logic and population count (\texttt{\_\_popc}) hardware intrinsics, achieving 98.6\% accuracy on MNIST and 89.8\% on CIFAR-10. To mitigate quantization loss, Rastegari et al.~\cite{rastegari2016xnor} proposed XNOR-Net by introducing analytic real-valued channel scaling factors ($\alpha$), while Liu et al.~\cite{qin2020bireal} introduced Bi-Real Net with FP32 real-valued identity shortcuts ($\mathbf{y} = \text{sign}(\mathbf{x}) * \mathbf{W}_b + \mathbf{x}$) to preserve continuous gradient flow, reaching Top-1 ImageNet accuracies of 51.2\% and 56.4\%, respectively. Further advancing BNN expressiveness, ReActNet~\cite{liu2020reactnet} introduced learnable per-channel activation shifts via RPReLU ($\text{RPReLU}(x) = \text{PReLU}(x + \gamma) + \beta$), IR-Net~\cite{qin2020irnet} proposed Libra-PB for information-entropy-balanced weight binarization, and AdaBin~\cite{tu2022adabin} fitted adaptive binary scaling sets, enabling ReActNet to reach 69.4\% Top-1 accuracy on ImageNet. Extending binary compression to dense prediction, Zhuang et al.~\cite{zhuang2019structured} presented Group-Net, a structured binary network that divides binary operations into multiple parallel branches with group fusion to reconstruct continuous spatial feature maps, achieving 63.8\% mIoU on Cityscapes road segmentation and 68.1\% mIoU on PASCAL VOC under 1-bit quantization.

Our research builds directly upon these complementary paradigms by bridging expressiveness and extreme compression: first, we adapt U-KAN for dense road segmentation by integrating multiple efficient KAN variant kernels (FasterKAN, ReLU-KAN, HardSwish-KAN, PWLO-KAN, TeLU-KAN) into the U-Net architecture to evaluate their impact on segmentation mIoU and latency; second, we propose BiKA (Binarized KAN), combining the per-connection activation philosophy of KANs with state-of-the-art BNN techniques (RPReLU, Libra-PB, AdaBin, FP32 shortcuts) to achieve ultra-lightweight, hardware-accelerated 1-bit road segmentation.
